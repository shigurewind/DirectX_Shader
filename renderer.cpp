...
ID3D11Buffer* g_LightBuffer = NULL;//ライト構造体転送用
ID3D11Buffer* g_WorldBuffer = NULL;//ワールド行列転送用
ID3D11Buffer* g_CameraBuffer = NULL;//カメラ行列転送用
...
void SetWorldMatrix( XMMATRIX WorldMatrix )
{
	...
	GetDeviceContext()->UpdateSubresource(g_WorldViewProjection, 0, NULL, &matrix, 0, 0);

	//ワールド行列のみをシェーダーへ転送
	worldViewProjection = XMMatrixTranspose(WorldMatrix);//ワールド行列を転置する
	XMStoreFloat4x4(&matrix, worldViewProjection);
	GetDeviceContext()->UpdateSubresource(g_WorldBuffer, 0, NULL, &matrix, 0, 0);
...
void SetCameraPosition(XMFLOAT3 CameraPisition)
{
	XMFLOAT4 temp = XMFLOAT4(CameraPisition.x, CameraPisition.y, CameraPisition.z, 0.0f);
...
//	rd.CullMode = D3D11_CULL_FRONT;	//表面をカリングする（表面は表示しない）
	...
	g_D3DDevice->CreateBuffer( &hBufferDesc, NULL, &g_WorldViewProjection );
	g_ImmediateContext->VSSetConstantBuffers( 0, 1, &g_WorldViewProjection);

	//ワールド行列オブジェクトをシェーダーへ接続　b2をつかう
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_WorldBuffer);
	g_ImmediateContext->VSSetConstantBuffers(2, 1, &g_WorldBuffer);//2は間違いないように
	...
	//ライトオブジェクトをシェーダーへ接続　b3を使う
	hBufferDesc.ByteWidth = sizeof(LIGHT);//転送データのサイズ
	g_D3DDevice->CreateBuffer(&hBufferDesc, NULL, &g_LightBuffer);
	g_ImmediateContext->VSSetConstantBuffers(3, 1, &g_LightBuffer);//b3
	g_ImmediateContext->PSSetConstantBuffers(3, 1, &g_LightBuffer);//b3

	//カメラ行列オブジェクトをシェーダーへ接続　b4を使う
	hBufferDesc.ByteWidth = sizeof(XMFLOAT4);//転送データのサイズ
