...
Texture2D		g_TextureNormal : register( t1 );		//ノーマルテクスチャ変数
   		   ...
   		   out float4 outDiffuse	: SV_Target )	//最終的なピクセルのカラー
{
    ...
    float4 normalMap;
    float4 normal;
    //法線マップからRGB値を取得して法線を計算
    normalMap = g_TextureNormal.Sample(g_SamplerState, inTexCoord);
    //RGBー＞ベクトル値へ
    normalMap = (normalMap * 2.0f) - 1.0f; //RGB値を-1.0～1.0へ変換
    ...
    specular = pow(specular, 20.0f); //ここでは30乗してみる
