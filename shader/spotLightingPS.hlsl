...
void main( in float4 inPosition : SV_POSITION, //変換後頂点座標
		   in float4 inWorldPosition : POSITION0, //ワールド変換済頂点座標
   		   ...
   		   out float4 outDiffuse	: SV_Target )	//最終的なピクセルのカラー
{
	...
	//================================
	//コーンの向きとlvの角度
    float angle = acos(dot(normalize(lv.xyz), normalize(Light.Direction.xyz)));
 	//angleがコーンの中心からどのくらい開いているかによる減衰	
    float spot = 1.0f - (1.0f / Light.Angle.x) * abs(angle);
    spot = max(0, spot);
    spot *= Light.PointLightParam.y; //明るさ補正
	//================================
	
	//テクスチャの色を取得
    outDiffuse = g_Texture.Sample(g_SamplerState, inTexCoord);
	//色と明るさを合成
    outDiffuse.rgb *= inDiffuse.rgb * Light.Diffuse.rgb * light * spot;
    ...
    outDiffuse.rgb += (specular * ofs * spot); //スペキュラをデフューズ値に加算する
